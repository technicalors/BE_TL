...
class ProductionPlanController extends AdminController
{
    ...
    public function storeProductionPlanAuto(Request $request)
    {
        ...
        $plans = ProductionPlan::where('ngay_sx', date('Y-m-d'))->whereIn('status_plan', [ProductionPlan::STATUS_COMPLETED, ProductionPlan::STATUS_IN_PROGRESS, ProductionPlan::STATUS_PENDING])->groupBy('lo_sx', 'machine_id')->get();
        foreach ($plans as $plan) {
            ...
            if ($productionOrderHistory->order_quantity - $productionOrderHistory->produced_quantity > 0) {
                ...
                $productionPlans[] = [
                    ...
                    'pass_input_lot_id' => $productionOrderHistory->line_id == 26 ? 1 : 0,
                ];
        ...
        foreach ($productionOrderPriorities as $productionOrderPriority) {
            ...
            foreach ($sortedHistories as $key => $history) {
                ...
                $productionPlans[] = [
                    ...
                    'pass_input_lot_id' => $history->line_id == 26 ? 1 : 0,
                ];
